 ...
 *
 * Clients that registered via CommandArbiter::Watch() receive these events
 * when another client acquires, preempts, or releases authority over a drone.
...
struct AuthorityEvent {
    ...
    enum class Kind : std::uint8_t {
        ...
        kResumed,    ///< The higher-priority client released; authority returned.
...
// pushes events from whichever thread calls CheckAndGrant() or Release().
// The handler pops and writes to the gRPC stream without holding any
// arbiter-internal lock, avoiding deadlock with the gRPC stream mutex.
// ---------------------------------------------------------------------------
...
class EventQueue {
   public:
     ...
     */
    bool Pop(AuthorityEvent& out, std::chrono::milliseconds timeout);
 ...
 * preempts the current holder, all registered watchers are notified
 * asynchronously so they can pause their command loops.
 *
 * @par Priority levels
 ...
 * never calls Release().  This prevents a crashed client from permanently
 * blocking a drone.
 */
class CommandArbiter {
   public:
     ...
     *   the holder is preempted and all registered watchers are notified.
     ...
     * this is a no-op.  On success all registered watchers at lower priority
     * levels receive an AuthorityEvent::Kind::kResumed notification.
     *
     ...
     */
    WatchToken Watch(const std::string&            drone_id,
   ...
   private:
    ...
    struct DroneState {
        // Current authority holder.  Empty = no holder.
        struct Holder {
            std::string     client_id;
            CommandPriority priority;
    ...
    /// @brief Notify all live watchers for @p drone_id without holding the lock.
    ...
    /// @brief Evict expired authority holders.  Called under @p lock.
    void EvictExpiredHolder(DroneState& state, const std::string& drone_id);
