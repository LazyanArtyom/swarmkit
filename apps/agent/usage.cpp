...
void PrintUsage() {
    std::cout << "Usage: swarmkit-agent [OPTIONS]\n"
                 ...
                 "  --bind      HOST:PORT     Bind address    (default: 127.0.0.1:50061)\n"
                 "  --config    PATH          Load agent config from YAML file\n"
                 "  --evidence-file PATH      Write canonical checksummed protobuf evidence\n"
                 "  --evidence-run-id ID      Operational recording identity\n"
                 "  --evidence-scenario-id ID Deployment/configuration identity\n"
                 "  --evidence-seed N         Record deterministic scenario seed\n"
                 "  --evidence-overwrite      Explicitly replace an existing evidence file\n"
                 ...
                 "  --mavlink-autopilot NAME  ardupilot-copter|ardupilot-plane\n"
                 "  --mavlink-target-system N MAVLink target system id\n"
                 "  --mavlink-target-component N\n"
                 "                             MAVLink target component id\n"
                 "  --mavlink-set-guided-before-arm true|false\n"
                 "                             Switch to configured guided mode before arm\n"
                 "  --mavlink-set-guided-before-takeoff true|false\n"
                 "                             Switch to configured guided mode before takeoff\n"
