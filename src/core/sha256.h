...
[[nodiscard]] constexpr std::uint32_t Choose(std::uint32_t choice_bits, std::uint32_t when_set,
                                             std::uint32_t when_clear) {
    return (choice_bits & when_set) ^ (~choice_bits & when_clear);
}

[[nodiscard]] constexpr std::uint32_t Majority(std::uint32_t first_bits, std::uint32_t second_bits,
                                               std::uint32_t third_bits) {
    return (first_bits & second_bits) ^ (first_bits & third_bits) ^ (second_bits & third_bits);
}

[[nodiscard]] constexpr std::uint32_t BigSigma0(std::uint32_t word) {
    return std::rotr(word, 2) ^ std::rotr(word, 13) ^ std::rotr(word, 22);
}

[[nodiscard]] constexpr std::uint32_t BigSigma1(std::uint32_t word) {
    return std::rotr(word, 6) ^ std::rotr(word, 11) ^ std::rotr(word, 25);
}

[[nodiscard]] constexpr std::uint32_t SmallSigma0(std::uint32_t word) {
    return std::rotr(word, 7) ^ std::rotr(word, 18) ^ (word >> 3);
}

[[nodiscard]] constexpr std::uint32_t SmallSigma1(std::uint32_t word) {
...
class Sha256 {
   ...
   private:
    void Transform(const std::uint8_t* block) {
        ...
        auto work_a = state_[0];
        auto work_b = state_[1];
        auto work_c = state_[2];
        auto work_d = state_[3];
        auto work_e = state_[4];
        auto work_f = state_[5];
        auto work_g = state_[6];
        auto work_h = state_[7];

        for (std::size_t index = 0; index < 64; ++index) {
            const std::uint32_t temp1 = work_h + BigSigma1(work_e) +
                                        Choose(work_e, work_f, work_g) + kRoundConstants[index] +
                                        schedule[index];
            const std::uint32_t temp2 = BigSigma0(work_a) + Majority(work_a, work_b, work_c);
            work_h = work_g;
            work_g = work_f;
            work_f = work_e;
            work_e = work_d + temp1;
...
[[nodiscard]] inline std::string Sha256FileHex(const std::filesystem::path& path) {
    ...
    Sha256 hasher;
    std::array<char, std::size_t{64} * std::size_t{1024}> buffer{};
    while (input.good()) {
